...
class QualityFlagsResponse(BaseModel):
    """Ответ с флагами качества данных"""
    flags: dict[str, bool] = Field(
        ...,
        description="Флаги качества данных"
    )
...
@app.post(
    ...
    tags=["quality"],
    summary="Флаги качества данных по CSV-файлу с использованием EDA-ядра",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    flags_bool: dict[str, bool] = {}
    for flag, val in flags_all.items():
        if type(val) == bool:
            flags_bool[flag] = val
